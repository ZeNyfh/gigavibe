...
discord command voteskip:
	...
	trigger:
		...
		if {_a} is not set:
			...
			add 1 to {_temp}
			set {_members::*} to members in the voice channel of event-member in event-guild
			loop {_members::*}:
			...
			set {_halfmembercount} to {_temp} / 2
			make embed:
				set {_B::1} to "✅ `You voted to skip the current track.`"
				set footer of embed to "%{_votecount}% of %{_halfmembercount}% needed to skip"
				set description of embed to join {_B::*} with nl
				set color of embed to color from rgb 32, 34, 37
			send last embed to event-channel
			if {_votecount} = {_halfmembercount}:
				make embed:
				...
				exit
			else if {_votecount} > {_halfmembercount}:
				make embed:
					set description of embed to ":fast_forward: `Skipped`"
					set color of embed to color from rgb 32, 34, 37
				send last embed to event-channel
...
discord command whitelist <text>:
	...
	trigger:
		if {blockeddj::*} contains discord id of event-member:
			...
			if {blockeddj::*} contains {_a}:
				remove {_a} from {blockeddj::*}
				make embed:
					set description of embed to "✅ `whitelisted %{_a}%.`"
					set color of embed to color from rgb 32, 34, 37
					send last embed to event-channel#
				exit
	...
	trigger:
		if {check} is set:
			...
			if {_vc} is not set:
				...
				run batch cmd "C:\Users\david\Desktop\code_stuff\discordbot\plugins\TTS.bat"
				wait 1 second
...
command /starta:
	trigger:
		if {tokentroll} is set:
			send message "I am already running" to the console
			exit
		else:
			login to {tokentroll} with name "Trollvibe"
